                              ...
                              <ng-template pTemplate="content">
                                <form [formGroup]="estadoForm" class="p-fluid">
                                  <div class="field">
                                    ...
                                    <p-select id="nuevoEstado" formControlName="nuevoEstado"
                                              [options]="estadoLista" optionLabel="descripcion" placeholder="Seleccionar" appendTo="body">
                                    </p-select>
                                    <div *ngIf="estadoForm.get('nuevoEstado')?.touched && estadoForm.get('nuevoEstado')?.invalid" class="p-error">
  ...
  formValidar() {
   ...
   /* if (dataObjeto.material.tipoRecurso) {
    ...
    this.form = this.fb.group({
      ...
      maxHoras: [dataObjeto.maxHoras ?? null, [
        Validators.required,
        Validators.min(1),
...
private mapToEquipo(obj: any): Equipo {
  ...
  nuevoRegistro() {
    // Suponiendo que "sedeFiltro" contiene la sede seleccionada y "0" es el id de "TODAS LAS SEDES".
      if (this.sedeFiltro && this.sedeFiltro.id === 0) {
        this.messageService.add({
          severity: 'warn',
          summary: 'Atención',
          detail: 'Debe seleccionar una sede para crear un nuevo equipo.'
        });
        return; // Evita abrir el modal
      }

      // Si la sede seleccionada es distinta de "TODAS LAS SEDES",
      // continuamos con la lógica normal (abrir el modal, etc.).
      this.modalEquipo.openModal();
  }
  ...
  deleteRegistro(objeto: Ejemplar) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.genericoService.conf_event_delete_1(objeto.idEquipo, 'api/equipos/delete')
          .subscribe(result => {
            if (result.status === 0) {
              this.objetoDialog = false;
