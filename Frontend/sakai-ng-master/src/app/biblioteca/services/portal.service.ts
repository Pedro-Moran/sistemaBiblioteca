    ...
    // Listar, con parámetros opcionales ?start=yyyy-MM-dd&end=yyyy-MM-dd
      ...
      saveNoticia(formData: FormData): Observable<ResponseDTO<void>> {
        return this.http.post<ResponseDTO<void>>(
          `${this.apiUrl}/api/noticias/registrar`,
          formData,
          { headers: this.authHeaders() }
        );
      }
      ...
      delete(id: number): Observable<{p_status:number}> {
        return this.http.delete<{p_status:number}>(
          `${this.apiUrl}/api/noticias/eliminar/${id}`,
          { headers: this.authHeaders() }
        );
      }

      // portal.service.ts
      toggleEstado(noticiaId: number, estadoId: number, idUsuario: number):
...
listarHorarios(): Observable<{ p_status: number; message: string; data: PortalHorario[] }> {
  return this.http.get<{ p_status: number; message: string; data: PortalHorario[] }>(
    ...
    // 4) CAMBIAR ESTADO
    toggleHorario(id: number, nuevoEstado: number, usuario: number):
            Observable<ResponseDTO<void>> {
            const body = {
