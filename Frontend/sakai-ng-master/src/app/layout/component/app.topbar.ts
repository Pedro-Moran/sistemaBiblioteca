...
@Component({
    selector: 'app-topbar',
    standalone: true,
    imports: [RouterModule, CommonModule, StyleClassModule, StyleClassModule, OverlayPanelModule, AppConfigurator],
            ...
            <div class="layout-topbar-menu hidden lg:block">
                <div class="layout-topbar-menu-content">
                    ...
                    <p-overlayPanel #op [dismissable]="true">
                      <ng-container *ngIf="notificaciones.length; else noMsgs">
                        <div *ngFor="let not of notificaciones; let i = index" class="flex items-center gap-2 p-2 border-b">
                          <i class="pi pi-envelope text-blue-600"></i>
                          <div class="flex-1">
                            <div class="font-medium">{{ not.mensaje }}</div>
                            <small class="text-xs text-gray-500">{{ not.fechaCreacion | date:'short' }}</small>
                          </div>
                          <button
                            pButton
                            icon="pi pi-check"
                      ...
                      <ng-template #noMsgs>
                        <div class="p-2 text-center text-gray-600">No tienes notificaciones nuevas</div>
                      </ng-template>
                    </p-overlayPanel>
                </div>
            </div>
...
export class AppTopbar implements OnInit {
    ...
    constructor(public layoutService: LayoutService, private prestamoService: PrestamosService) {}
    ...
    ngOnInit() {
     ...
     loadNotifications() {
      ...
      leer(id: number, index: number) {
        this.prestamoService.marcarLeida(id).subscribe(() => {
          // quitar la notificación marcada
    ...
    onNotificacionClick(n: Notificacion) {
        // Marca como leída
        this.prestamoService.marcarLeida(n.id).subscribe(() => {
          // Quitarla del array
          this.notificaciones = this.notificaciones.filter(x => x.id !== n.id);
        });
        // Aquí rediriges si quieres, ej:
