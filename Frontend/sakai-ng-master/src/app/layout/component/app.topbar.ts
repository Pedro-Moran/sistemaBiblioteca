          ...
          <p-table [value]="solicitudes" *ngIf="solicitudes.length">
            ...
            <ng-template pTemplate="body" let-s let-i="rowIndex">
              <tr [ngClass]="{ 'highlight-row': highlightSolicitudes.includes(i) }">
                <td>{{ s.equipo?.nombreEquipo || s.material?.titulo }}</td>
                <td>{{ s.fechaPrestamo ? 'Aceptado' : 'Rechazado' }}</td>
                <td>{{ s.fechaPrestamo ? (s.fechaPrestamo | date:'short') : '-' }}</td>
              </tr>
            </ng-template>
          </p-table>
...
export class AppTopbar implements OnInit {
  ...
  abrirSolicitudes() {
    this.prestamoService.getMisPrestamos().subscribe({
      next: resp => {
        const lista = resp.data as DetallePrestamo[];
        this.solicitudes = lista
          .filter(s => {
            const desc = s.estado?.descripcion?.toUpperCase() || '';
            // exclude pendientes
            return desc !== 'RESERVADO';
          })
          .sort((a, b) => {
